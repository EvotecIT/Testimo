...
| Domain Controller | Resolves internal DNS queries       | Connectivity | Verify `DNS on DC [resolves Internal DNS]`                                                   | Tested          | Resolve $Domain, requires to match ALL DC's IPV4                    |
| Domain Controller | Resolves external DNS queries       | Connectivity | Verify `DNS on DC [resolves External DNS]`                                                   | Tested          | Resolve evotec.xyz - requires to match IPV4                         |
